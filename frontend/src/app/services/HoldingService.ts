...
export class HoldingService {
  ...
  createHolding(holding: Holding): Observable<Holding> {
    return this.http
      ...
      .pipe(catchError(() => throwError(() => new Error('Failed to create Holding.'))));
  }
  ...
  deleteHolding(id: HoldingId): Observable<void> {
    return this.http
      ...
      .pipe(catchError(() => throwError(() => new Error('Failed to delete Holding.'))));
  }
  ...
  getUserHoldingTotal(userId: number): Observable<number> {
    ...
    return this.http
      ...
      .pipe(catchError(() => throwError(() => new Error('Failed to load total user holdings.'))));
  }
  ...
  totalInvestedCost(userId: number): Observable<number> {
    ...
    return this.http
      .get<number>(this.URL + '/totalInvestedCost', { params })
      .pipe(
        catchError(() =>
          throwError(() => new Error('Failed to load total invested cost for specified User.')),
        ),
      );
  }
}
